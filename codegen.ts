...
const OPTIMIZELY_FORMS_ENABLED =
    process.env.OPTIMIZELY_FORMS_ENABLED === 'true';
...
function buildDocumentArray() {
    ...
    if (OPTIMIZELY_DAM_ENABLED && !OPTIMIZELY_FORMS_ENABLED) {
        ...
        const damFiles = allGraphqlFiles.filter((file) =>
            file.endsWith('.dam.graphql')
        );
        ...
        const allCmsFiles = glob.sync('./src/cms/**/*.graphql');
        const allGraphqlFiles = [...graphqlFiles, ...allCmsFiles];
        const formsFiles = allGraphqlFiles.filter((file) =>
            file.endsWith('.forms.graphql')
        );
        const regularFiles = allGraphqlFiles
            .filter((file) => !file.endsWith('.dam.graphql'))
            .filter((file) => !file.endsWith('.forms.graphql'));
        ...
        // For each regular file, check if a DAM version exists
        const finalFiles = regularFiles.filter((regularFile) => {
            const formsVersion = regularFile.replace(
                '.graphql',
                '.forms.graphql'
            );
        ...
        const finalFiles = regularFiles.filter((regularFile) => {
            ...
            return (
                !formsFiles.includes(formsVersion) &&
                !damFiles.includes(damVersion)
            );
        });
...
const config: CodegenConfig = {
    ...
    generates: {
        ...
        './src/pages/tools/codegen/graphql/': {
            preset: 'client',
            documents: ['src/pages/tools/queries/*.ts'], 
            config: {
                documentMode: 'string',
                useTypeImports: true,
            },
        },
        './src/pages/tools/codegen/schema.graphql': {
            plugins: ['schema-ast'],
            documents: ['src/pages/tools/queries/*.ts'],
            config: {
